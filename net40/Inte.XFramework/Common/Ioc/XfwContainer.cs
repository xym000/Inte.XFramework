...
namespace Inte.XFramework
{
    ...
    public sealed class XfwContainer : IContainer
    {
        ...
        {
            //if (isSingleton)
            //    SingletonCache<T>.Instance = func();
            //FuncCache<T>.Instance = func;
        ...
        {
            //T instance1 = SingletonCache<T>.Instance;
            //if (instance1 != null) return instance1;

            //Func<T> creator = FuncCache<T>.Instance;
            //if (creator == null) 
            //    XfwException.Throw(typeof(T).FullName + " is not registered");
            
            //return creator();

            Activator activator;
            if (!_cache.TryGet(typeof(T), out activator)) 
                throw new XfwException(typeof(T).FullName + " is not registered");
