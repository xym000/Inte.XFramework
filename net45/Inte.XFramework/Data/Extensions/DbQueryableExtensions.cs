...
namespace Inte.XFramework.Data
{
    ...
    public static class DbQueryableExtensions
    {
        ...
        {
            IDbQueryable<int> query = source.CreateQuery<int>(DbExpressionType.Count, predicate);
            return await query.Provider.ExecuteAsync(query);
        }
        ...
        {
            List<TElement> elementList = await source.Provider.ExecuteListAsync(source);
            return elementList;
        }
        ...
        {
            string cmd = source.ToString();
            return await source.Provider.ExecuteDataTableAsync(cmd);
        }
        ...
        {
            string cmd = source.ToString();
            return await source.Provider.ExecuteDataSetAsync(cmd);
        }
